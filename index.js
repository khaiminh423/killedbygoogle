const [searchInput, filterSelect, gridCont] = document.querySelectorAll('input,select,main');
const phrases = [
  'Sentenced to death',
...
//------------------------------------ SELECT OPTIONS --------------------------------------------
const renderOption = (value, textContent) => h('option', { value }, textContent);

const renderSelect = (grave) => filterSelect.append(renderOption('any', `All (${grave.length})`), ...Object.entries(Object.groupBy(grave, (item) => item.type)).map(([type, items]) => renderOption(type, `${type} (${items.length})`)));
    ...
    .map((stone) => {
      ...
      return h(
        ...
        h(
          ...
          h('time', null, isFuture ? itemDate.toLocaleString('en-US', { month: 'long', year: 'numeric' }).replace(' ', '\n') : `${stone.dateOpen.split('-')[0]} - ${stone.dateClose.split('-')[0]}`),
          h('div', null, stone.type)
        ),
        h('div', null, h('h2', null, h('a', { href: stone.link, target: '_blank' }, stone.name)), h('div', null, `${killedText}, ${stone.description}. ${isFuture ? 'It will be' : 'It was'} ${Math.abs(itemAge[0])} ${itemAge[1]}${Math.abs(itemAge[0]) === 1 ? '' : 's'} old.`))
      );
    });

  gridCont.replaceChildren(h('aside'), ...graveStones);
};
//------------------------------------------------------------------------------------------------
...
const humanDistance = (dateCompare) => {
  ...
  if (Math.abs(delta) < MONTH) return [(delta / DAY) | 0, 'day'];
  if (Math.abs(delta) < YEAR) return [(delta / MONTH) | 0, 'month'];
